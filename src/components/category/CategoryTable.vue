...
// Setup table headers
const headers = [
  { title: 'ชื่อหมวดหมู่', key: 'name', align: 'start' as const, sortable: true },
...
computed(() => {
  table.setTotalItems(props.totalCategories);
  return props.totalCategories;
});
  ...
  >
    ...
    <template #no-data>
      <v-sheet class="pa-6 text-center">
        <v-icon size="64" icon="mdi-shape-outline" class="mb-4"></v-icon>
        <div class="text-h6 mb-2">ไม่พบข้อมูลหมวดหมู่</div>
        <div class="text-body-2 text-medium-emphasis mb-4">
          ยังไม่มีข้อมูลหมวดหมู่ในระบบ หรือไม่พบข้อมูลที่ตรงกับการค้นหา
        </div>
      </v-sheet>
    </template>
  </BaseTable>
